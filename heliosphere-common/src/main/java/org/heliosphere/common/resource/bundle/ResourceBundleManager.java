...
import org.heliosphere.common.exception.InvalidArgumentException;
import org.heliosphere.common.exception.ResourceBundleException;
...
/**
 * The resource bundle manager to discover, register and access the resource bundles.
 * <hr>
...
public final class ResourceBundleManager
{
	...
	public static final String getMessage(final Enum<? extends IBundle> key, final Object... parameters)
	{
		if (key == null)
		{
			throw new InvalidArgumentException(BundleHeliosphereCommon.ResourceBundleInvalidKey);
		}
	...
	private static final void initialize(final String filename, final Locale locale)
	{
		...
		{
			throw new InvalidArgumentException(BundleHeliosphereCommon.ResourceBundleInvalidName);
		}

		if (locale == null)
		{
			throw new InvalidArgumentException(BundleHeliosphereCommon.ResourceBundleInvalidLocale);
		}
	...
	public static final void register(final String filename)
	{
		...
		{
			throw new ResourceBundleException(BundleHeliosphereCommon.ResourceBundleNotFound, filename, getLocale(), e);
		}
	}
	...
	private static final void register(final PropertiesConfiguration properties)
	{
		...
		if (properties == null)
		{
			throw new InvalidArgumentException(BundleHeliosphereCommon.ResourceBundleInvalidConfiguration);
		}
	...
	private static final void register(final Class<? extends IBundle> bundleClass, final ResourceBundle bundle)
	{
		...
		try
		{
			if (!BUNDLES.containsKey(bundleClass))
			{
				...
				log.debug(getMessage(BundleHeliosphereCommon.ResourceBundleRegistered, bundleClass.getSimpleName(), locale.toString(), Integer.valueOf(bundle.keySet().size()), filename));
			}
			else
			{
				/*
				...
				{
					log.debug(getMessage(BundleHeliosphereCommon.ResourceBundleAlreadyRegistered, bundleClass.getSimpleName(), locale.toString()));
				}
				else
				{
					BUNDLES.put(bundleClass, bundle);

					log.debug(getMessage(BundleHeliosphereCommon.ResourceBundleReplaced, bundleClass.getSimpleName(), previous.getLocale().toString(), locale.toString(), Integer.valueOf(bundle.keySet().size())));
				}
			}
		}
	...
	public static final String getResourceForMethodName(@NonNull final Class<? extends Enum<?>> eClass, final Enum<?> e, final Locale locale)
	{
		...
		throw new ResourceBundleException(BundleHeliosphereCommon.ResourceBundleInvalidKey, null, key, null, locale, e);
	}
}
